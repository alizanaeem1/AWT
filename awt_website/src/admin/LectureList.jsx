import { BookOpen, Box, CalendarDays, Edit3, Eye, FileText, Plus, Search, Send, SlidersHorizontal, Trash2 } from 'lucide-react'
import { useMemo, useState } from 'react'
import { Link } from 'react-router-dom'
import ConfirmDialog from '../components/ConfirmDialog.jsx'
import { useAdminLectures } from '../hooks/useAdminData.js'
import { useToast } from '../hooks/useToast.js'
import { deleteLecture, fetchAdminLectures, setLecturePublished } from '../lib/adminRepository.js'
  ...
  return (
    <>
      ...
      <AdminCard className="mt-5 overflow-hidden p-0">
        {isLoading ? <AdminLoadingLabel label="Loading lectures" /> : null}
          ...
          <table className="w-full min-w-[880px] text-left text-sm">
            <thead className="border-b border-slate-800 bg-slate-950/60 text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-5 py-4">Lecture Title</th>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-slate-500">{lecture.slug || 'lecture-page'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4"><StatusBadge status={lecture.status} /></td>
                  <td className="px-5 py-4 text-slate-400">{lecture.updatedAt}</td>
                  <td className="px-5 py-4 text-slate-300">{lecture.blocks ?? 0}</td>
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      <Link to={lecture.slug ? `/lectures/${lecture.slug}` : '/'} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-800 text-blue-300 hover:border-blue-400 hover:bg-blue-400/10" title="Preview lecture">
